...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 h-16 transition-all duration-300 ${scrolled ? "bg-[rgba(4,13,30,0.92)] backdrop-blur-xl" : ""}`}
    >
      {/* 3-column grid: logo | links (centered) | actions */}
      <div
        className="hidden md:grid h-full px-8 items-center"
        style={{ gridTemplateColumns: "1fr auto 1fr" }}
      >
        {/* Logo — left */}
        <a
          href="#hero"
          className="font-mono text-cyan text-base tracking-wide justify-self-start"
        >
      ...
      <div className="md:hidden flex items-center justify-between h-full px-5">
        <a href="#hero" className="font-mono text-cyan text-base">
          Rafael<span className="text-blue">.AC</span>
        </a>
        <button
        ...
        >
          <span
            className={`w-5 h-0.5 bg-[var(--text)] block transition-all ${menuOpen ? "rotate-45 translate-y-2" : ""}`}
          />
          <span
            className={`w-5 h-0.5 bg-[var(--text)] block transition-all ${menuOpen ? "opacity-0" : ""}`}
          />
          <span
            className={`w-5 h-0.5 bg-[var(--text)] block transition-all ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`}
          />
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 w-full bg-[var(--card)] border-b border-[var(--border)] flex flex-col p-4 gap-2 md:hidden z-40">
          ...
          <div className="flex gap-2 mt-2">
            <button
              onClick={() => setLang(lang === "es" ? "en" : "es")}
              className="flex-1 border border-[var(--border)] rounded-lg py-2 text-sm font-mono font-bold text-cyan"
            >
              {lang.toUpperCase()}
            </button>
          </div>
