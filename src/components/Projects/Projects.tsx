...
const ProjectCard = ({ project, delay, ctaLabel }: ProjectCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        <div className="font-mono text-[0.72rem] text-cyan uppercase tracking-[0.1em] mb-1.5">{project.type}</div>
        <h3 className="font-syne text-[1.05rem] font-bold mb-2">{project.title}</h3>
        <p className="text-[var(--text-muted)] text-[0.85rem] leading-[1.55] mb-4">{project.desc}</p>
        <span className="inline-flex items-center gap-1 text-[0.85rem] text-blue font-semibold group-hover:gap-2 transition-all">
          {ctaLabel} <span className="transition-transform group-hover:translate-x-1">→</span>
        </span>
      </div>
    </a>
  )
    ...
    <section id="projects" className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <div ref={header.ref} className={`reveal ${header.visible ? 'visible' : ''}`}>
          <div className="font-mono text-[0.78rem] text-cyan uppercase tracking-[0.15em] mb-3">{translations.projects.tag}</div>
          <h2 className="font-syne text-[clamp(1.9rem,4vw,2.7rem)] font-extrabold leading-[1.15] mb-4">
            {translations.projects.title}<br/><span className="grad">{translations.projects.titleGrad}</span>
          </h2>
          <p className="text-[var(--text-muted)] text-[1rem] max-w-[540px] mb-12 leading-[1.7]">
            {translations.projects.sub}
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {translations.projects.items.map((project, projectIndex) => <ProjectCard key={project.title} project={project} delay={projectIndex % 3} ctaLabel={translations.projects.cta}/>)}
        </div>
