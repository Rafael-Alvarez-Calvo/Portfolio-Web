...
const logos: Record<string, (p: LogoProps) => JSX.Element> = {
  caixabank: ({ className }) => (
    <img src="/logo-caixabank.png" alt="CaixaBank Tech" className={`object-contain ${className}`}/>
  ),
  mentorup: ({ className }) => (
    <div className={`flex items-center ${className}`}><MentorUpLogo/></div>
  ),
  pisos: ({ className }) => (
    <div className={`flex items-center gap-2 ${className}`}>
      <img src="https://statics.imghs.net/dist/img/pisos-logo-white.svg"
        alt="Pisos.com" className="h-5 object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display='none' }}/>
      <span className="text-[var(--text-dim)] text-xs">·</span>
      <img src="https://www.habitatsoft.com/images/assets/svg/logoHS.svg"
        alt="Habitatsoft" className="h-5 object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display='none' }}/>
    </div>
  ),
  paldental: ({ className }) => (
    <img src="/logo-paldental.png" alt="Clínica Paldental" className={`object-contain ${className}`}/>
  ),
  sidertia: ({ className }) => (
    <img src="/logo-sidertia.png" alt="Sidertia" className={`object-contain rounded-lg ${className}`}/>
  ),
}

const logoKeys = ['caixabank', 'mentorup', 'pisos', 'paldental', 'sidertia']

function TimelineItem({ job, index, currentLabel }: {
  job: { date:string; company:string; role:string; current:boolean; desc:string; tech:readonly string[] }
  index: number
  currentLabel: string
}) {
  ...
  return (
    <div ref={ref} className="timeline-item relative flex gap-8" style={{ transitionDelay: `${index * 0.08}s` }}>
      ...
      <div className="relative flex flex-col items-center flex-shrink-0" style={{ width: '20px' }}>
        <div className="relative z-10 rounded-full flex-shrink-0" style={{
          width: '14px', height: '14px', marginTop: '4px',
          background: job.current ? '#06b6d4' : '#3b82f6',
          boxShadow: job.current ? '0 0 0 3px rgba(6,182,212,0.2),0 0 16px rgba(6,182,212,0.5)' : '0 0 0 3px rgba(59,130,246,0.15),0 0 10px rgba(59,130,246,0.3)',
          border: '2px solid var(--bg2)',
        }}/>
        <div className="flex-1 w-px mt-2" style={{
      ...
      <div className="pb-10 flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-1.5">
          {job.tech.map(t => (
            <span key={t} className="bg-[rgba(59,130,246,0.07)] border border-[rgba(59,130,246,0.18)] rounded px-2 py-0.5 font-mono text-[0.72rem] text-[var(--text-muted)]">{t}</span>
          ))}
        </div>
      </div>
    </div>
      ...
      <div className="max-w-6xl mx-auto">
        <div ref={h.ref} className={`reveal ${h.visible ? 'visible' : ''}`}>
          ...
          <h2 className="font-syne text-[clamp(1.9rem,4vw,2.7rem)] font-extrabold leading-[1.15] mb-12">
            {t.experience.title}<br/><span className="grad">{t.experience.titleGrad}</span>
          </h2>
        </div>
        <div className="flex flex-col">
          {t.experience.jobs.map((job, i) => (
            <TimelineItem key={job.company + job.date} job={job} index={i} currentLabel={t.experience.current}/>
          ))}
        </div>
      </div>
    </section>
